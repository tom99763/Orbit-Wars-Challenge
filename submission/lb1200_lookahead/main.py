...
def _lb1200_base_agent(obs, config=None):
    global _agent_step
    # Reset counter when a new game starts (obs reports step 0).
    obs_step = _read(obs, "step", None)
    if obs_step == 0 or obs_step is None:
        _agent_step = 0
    _agent_step += 1
...
# [inlined above: __lb1200_base_agent, build_world, simulate_planet_timeline, etc.]
...
def agent(obs, config=None):
    ...
    primary = _lb1200_base_agent(obs, config) or []
